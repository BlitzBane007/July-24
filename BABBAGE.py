"""
This program helps with picking the download URL and folder for a specific DOCUMENT feed and then downloading the latest file.
...
Added functions:
Added the ability to compare the Checksum values in both the fields.
Returns the ISIN, document country, document language for the missing ISIN.
"""
import datetime
import os
import subprocess
    ...
    for path, delimiter, url in zip(paths, delimiters, urls):
        ...
        if os.path.exists(path):
            ...
            if len(files) >= 2:
                ...
                missing_checksums = pd.Series(list(set(df_hfi[primary_k].unique()) - set(df_efs[primary_k].unique())))
                print('Checksum missing in efs:', len(missing_checksums))
                head2 = f'Checksum missing in {file1_path}'
                missing_checksum_df = pd.DataFrame({head2: missing_checksums})
                output_data.append(missing_checksum_df)
                isin_values = list(df_hfi.loc[df_hfi['Checksum'].isin(missing_checksums), 'ISIN'])
                head5 = "Missing ISIN"
                missing_isins_df = pd.DataFrame({head5: isin_values})
                output_data.append(missing_isins_df)
                mask = df_hfi['Checksum'].str.contains('|'.join(missing_checksums), na=False)

                filtered_df = df_hfi[mask]

                required_columns = ["DocumentType", "Country", "Language"]
                isin_params = filtered_df[required_columns].copy()
                header_list = isin_params.columns.tolist()
                data_list = isin_params.to_dict(orient='records')
                param_df = pd.DataFrame(data_list, columns=header_list)
                output_data.append(param_df)

                if var_filter.get():
                    print("FILTER CHECK INITIATED")
                    api = extract_api_from_url(url)
                    for isin_value in tqdm(isin_values):
                        citicode = find_associated_value(df_enig, isin_value)
